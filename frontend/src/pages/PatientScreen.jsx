...
const PatientScreen = () => {
  ...
  useEffect(() => {
    if (trackingInfo && trackingInfo.status === 'WAITING') {
      const waitingList = activeQueue.filter(t => t.status === 'WAITING');
      const idx = waitingList.findIndex(t => t.tokenNumber === trackingInfo.tokenNumber);
      
      if (idx !== -1) {
        setTrackingInfo({
          ...trackingInfo,
          peopleAhead: idx,
          estimatedWaitTime: (idx + (currentlyCalledToken ? 1 : 0)) * averageConsultationTime
        });
      } else {
  ...
  const getJourneyStep = () => {
    ...
    if (trackingInfo.status === 'WAITING') return 2;
    return 1;
  }
  ...
  return (
    <div className="bg-surface text-on-background font-body-md min-h-screen flex flex-col overflow-x-hidden antialiased">
      {/* TopAppBar */}
      <header className="flex justify-between items-center w-full px-4 h-16 sticky top-0 z-50 bg-surface shadow-sm border-b border-outline-variant">
        <div className="flex items-center gap-2">
          <span className="material-symbols-outlined text-primary">local_hospital</span>
          <h1 className="text-xl font-bold text-primary tracking-tight">ClinQ</h1>
        </div>
        <div className="flex items-center gap-4">
            ...
            <div className="w-10 h-10 rounded-full bg-secondary-container text-secondary flex items-center justify-center shadow-sm relative">
              <span className="material-symbols-outlined text-[20px]">notifications</span>
              <div className="absolute top-2 right-2 w-2.5 h-2.5 bg-error rounded-full ring-2 ring-surface"></div>
            </div>
        </div>
      </header>

      <main className="flex-1 w-full max-w-md lg:max-w-6xl mx-auto p-4 lg:p-8 flex flex-col lg:grid lg:grid-cols-12 gap-5 lg:gap-8">
        
        {/* Left Column (Desktop) */}
        <div className="flex flex-col gap-5 lg:col-span-5 xl:col-span-5">
          {/* Now Serving Banner */}
          <div className="bg-surface-container-lowest border border-outline-variant rounded-2xl p-6 lg:p-10 text-center shadow-sm relative overflow-hidden mt-2 lg:mt-0 flex-1 flex flex-col justify-center">
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-[0.03] pointer-events-none">
              <span className="material-symbols-outlined text-[200px] lg:text-[300px]">campaign</span>
            </div>
            <p className="text-xs lg:text-sm font-bold text-on-surface-variant uppercase tracking-[0.2em] mb-4">Now Serving</p>
            <div className="inline-block bg-secondary/10 px-8 lg:px-12 py-3 lg:py-6 rounded-[32px] mb-6">
          ...
          <section className="mt-2 mb-4 lg:mb-0">
            <div className="rounded-2xl overflow-hidden h-44 lg:h-56 relative shadow-sm border border-outline-variant/30">
        ...
        <div className="flex flex-col gap-5 lg:col-span-7 xl:col-span-7">
          ...
          {trackingInfo && (
            <section className="grid grid-cols-1 gap-4">
              <div className="bg-primary-container text-white rounded-2xl p-6 lg:p-8 shadow-md relative overflow-hidden">
                <div className="relative z-10 flex justify-between items-center">
                  <div className="space-y-1 lg:space-y-2">
                    <p className="text-white/80 font-semibold text-sm lg:text-base">Your Token</p>
                    <h2 className="text-[56px] lg:text-[72px] font-black leading-none">{trackingInfo.tokenNumber}</h2>
                  </div>
                  <div className="text-right">
                ...
                <div className="mt-6 lg:mt-8 pt-5 lg:pt-6 border-t border-white/20 flex items-center justify-between relative z-10">
                  <div className="flex items-center gap-3 lg:gap-4">
                    <div className="h-10 w-10 lg:h-12 lg:w-12 rounded-full bg-white/20 flex items-center justify-center">
                      <span className="material-symbols-outlined text-[20px] lg:text-[24px]">person_search</span>
                    </div>
          ...
          {/* Queue Progress Bar */}
          {trackingInfo && (
            <section className="bg-surface-container-lowest rounded-2xl p-6 lg:p-8 border border-outline-variant shadow-sm">
              <h3 className="font-bold text-sm lg:text-base text-on-surface-variant mb-6 lg:mb-8 uppercase tracking-wider">Your Journey</h3>
              <div className="relative flex justify-between items-start pt-1 lg:px-8">
                {/* Track Line */}
                <div className="absolute top-5 lg:top-6 left-0 lg:left-8 w-full lg:w-[calc(100%-4rem)] h-[2px] bg-outline-variant">
                ...
                {/* Steps */}
                <div className="relative flex flex-col items-center gap-2 lg:gap-3 z-10">
                  <div className="w-10 h-10 lg:w-12 lg:h-12 rounded-full bg-secondary flex items-center justify-center text-white ring-[6px] ring-surface-container-lowest">
                    <span className="material-symbols-outlined text-[18px] lg:text-[20px]">check</span>
                  </div>
                  <span className="text-xs lg:text-sm font-semibold text-on-surface-variant">Arrival</span>
                </div>
                
                <div className="relative flex flex-col items-center gap-2 lg:gap-3 z-10">
                  <div className={`w-10 h-10 lg:w-12 lg:h-12 rounded-full flex items-center justify-center ring-[6px] ring-surface-container-lowest transition-colors ${journeyStep >= 2 ? 'bg-secondary text-white' : 'bg-surface-container text-outline-variant'} ${journeyStep === 2 ? 'animate-pulse' : ''}`}>
                    <span className="material-symbols-outlined text-[18px] lg:text-[20px]">hourglass_empty</span>
                  </div>
                  <span className={`text-xs lg:text-sm ${journeyStep >= 2 ? 'text-secondary font-bold' : 'text-on-surface-variant font-semibold'}`}>Waiting</span>
                </div>
                
                <div className="relative flex flex-col items-center gap-2 lg:gap-3 z-10">
                  <div className={`w-10 h-10 lg:w-12 lg:h-12 rounded-full flex items-center justify-center ring-[6px] ring-surface-container-lowest transition-colors ${journeyStep >= 3 ? 'bg-secondary text-white animate-pulse' : 'bg-surface-container text-outline-variant'}`}>
                    <span className="material-symbols-outlined text-[18px] lg:text-[20px]">medical_services</span>
                  </div>
                  <span className={`text-xs lg:text-sm ${journeyStep >= 3 ? 'text-secondary font-bold' : 'text-on-surface-variant font-semibold'}`}>Doctor</span>
                </div>
              </div>
            </section>
          )}
          ...
          <section className="space-y-4 mt-2 flex-1 flex flex-col">
            ...
            <div className="space-y-2.5">
              {recentlyCalled.length > 0 ? recentlyCalled.map((token, index) => (
                <div key={token._id} className={`flex items-center justify-between p-4 lg:p-5 bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm transition-opacity hover:border-secondary/30 ${index === 1 ? 'opacity-80' : index === 2 ? 'opacity-60' : ''}`}>
                  <div className="flex items-center gap-4">
                    <span className="text-xl lg:text-2xl text-on-surface font-black">{token.tokenNumber}</span>
                    <div className="w-[1px] h-5 lg:h-6 bg-outline-variant"></div>
                    <span className="text-sm lg:text-base font-medium text-on-surface-variant">Consultation Room 1</span>
                  </div>
